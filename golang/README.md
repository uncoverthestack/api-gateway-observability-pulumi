# API Observability Template — Go

The Go implementation of the API Gateway observability template, built with Pulumi.
...
```
golang/
├── main.go                           # Entry point — reads config, wires modules
├── Pulumi.yaml                       # Project definition
├── Pulumi.dev.yaml                   # Example config
├── go.mod                            # Go module file
├── observability/
│   ├── dashboard.go                  # CloudWatch dashboard (10 widgets)
│   ├── alarms.go                     # 5 alarm types with SNS actions
│   └── notifications.go              # SNS topic + email subscription
└── examples/
